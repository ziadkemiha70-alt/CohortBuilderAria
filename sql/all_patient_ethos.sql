...
SET NOCOUNT ON;
	...
	-- Création d'une table temporaire globale pour les patients ETHOS
IF OBJECT_ID('tempdb..#PatientsETHOS') IS NOT NULL DROP TABLE #PatientsETHOS;
SELECT DISTINCT tr.PatientSer
INTO #PatientsETHOS
FROM TreatmentRecord tr
JOIN Machine m ON tr.ActualMachineSer = m.ResourceSer
WHERE m.MachineId = 'ETHOS';
...
-- Table contenant les fractions de traitement détaillées pour chaque patient ETHOS
...
-- Insérer les données dans @FractionsChamps en filtrant sur les patients ETHOS
INSERT INTO @FractionsChamps
	...
	-- Filtrage ETHOS global
	WHERE TreatmentRecord.PatientSer IN (SELECT PatientSer FROM #PatientsETHOS);


--SELECT * FROM @FractionsChamps
------------------------------------------------------------------------
...
-- a voir la pertirnence de ce truc
...
--SELECT * FROM @ChampsSites; --WHERE IdPatient = '';
...
WHERE p.Status = 'Approved'
	AND c.PatientSer IN (SELECT PatientSer FROM #PatientsETHOS)

--select * from @Prescriptions where PatientSer = ''
------------------------------------------------------------------------

-- il faut que la machine soit lier au courseid
...
INNER JOIN #PatientsETHOS pe ON pe.PatientSer = P.PatientSer
...
WHERE PatientVolume.PatientSer IN (SELECT PatientSer FROM #PatientsETHOS)
--WHERE p.PatientId = ''
;
/*
select distinct * 
...
/*LEFT*/ JOIN CourseDiagnosis CourseDiagnosis ON CourseDiagnosis.CourseSer = Course.CourseSer -- je peux me baser la dessus 
...
/*LEFT*/ JOIN PlanSetup PlanSetup ON PlanSetup.CourseSer = Course.CourseSer

JOIN @PlanSession PlanSession ON PlanSession.PlanSetupSer = PlanSetup.PlanSetupSer
left JOIN VARIAN.dbo.ScheduledActivity ScheduledActivity ON ScheduledActivity.ScheduledActivitySer = PlanSession.ScheduledActivitySer
left JOIN VARIAN.dbo.ActivityCapture ActivityCapture ON ActivityCapture.ActivityInstanceSer = ScheduledActivity.ActivityInstanceSer
...
WHERE PlannedFrac > 0
AND Patient.PatientSer IN (SELECT PatientSer FROM #PatientsETHOS);
...
LEFT JOIN @PlanSession PS ON DPC.RTPlanSer = PS.RTPlanSer
