...
def _guess_csv_dialect(sample_text: str) -> csv.Dialect:
    """Détecte le séparateur CSV avec fallback robuste."""
    try:
    ...
    with _open_csv_text_stream(uploaded_file) as text:
        ...
        for row in reader:
            ...
            if len(rows) >= chunksize:
                yield pd.DataFrame(rows, columns=selected_cols, dtype=object)
                rows = []
        if rows:
            yield pd.DataFrame(rows, columns=selected_cols, dtype=object)
...
def read_uploaded_columns(uploaded_file) -> List[str]:
...
@st.cache_data(show_spinner=False)
def load_form_meta_only(uploaded_file) -> Tuple[pd.DataFrame, List[str]]:
